pipeline {
  ...
  stages {
    ...
    stage('Build and Push Docker Image') {
      environment {
        DOCKER_IMAGE = "stenythomas/ultimate-cicd:${BUILD_NUMBER}"
      }
      steps {
        script {
      ...
      steps {
        withCredentials([
            ...
            sh '''
                ...
                git config user.email "stenythomas163@gmail.com"
                git config user.name "${GIT_USER_NAME}"

                sed -i "s|image: .*|image: stenythomas/ultimate-cicd:${BUILD_NUMBER}|g" node-app-manifests/deployment.yml
